(() => {
  ...
  C.hero = item => `${C.image(item.banner_url || item.cover_url, item.title_en, true)}<header class="series-hero">
    <div class="series-heading"><div class="series-title-row"><h1>${C.e(item.title_en)}</h1>${C.statusBadge(item.status)}</div>
  ...
  C.overview = item => `<section id="series-overview" class="series-section"><div class="series-overview-layout${item.cast.length ? ' has-cast' : ''}"><div class="series-overview-story"><h2>ABOUT THE SERIES</h2>${C.text(item.description)}
    ${item.official_hashtag || item.links.length ? `<div class="series-social-row">${item.links.length ? C.links(item.links) : ''}${item.official_hashtag ? `<p class="series-hashtag">${C.hashtagLinks(item.official_hashtag)}</p>` : ''}</div>` : ''}</div>${C.cast(item.cast)}</div></section>`;
})();
